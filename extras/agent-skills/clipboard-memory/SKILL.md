...
Recall what the user copied on this Mac before reaching for generic search. `clipmem` maintains a local, privacy-preserving SQLite archive of every clipboard state macOS emits, and exposes a JSON-first CLI built for agents. There is a sibling OpenClaw-native package at `extras/openclaw/clipboard_memory/` that installs via `clipmem agents openclaw install-skill`; the content below applies to both.
