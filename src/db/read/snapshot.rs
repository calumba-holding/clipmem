...
use crate::db::types::{
    Database, RepresentationDerivativePayload, RepresentationManifest, RepresentationPayload,
    RestorePayloadPlan, SnapshotItemManifest, SnapshotMetadata,
};
use crate::model::{
...
impl Database {
    ...
    pub fn read_representation_preview(
        ...
        self.conn.query_row(
            ...
            |row| Ok(RepresentationDerivativePayload {
                source_raw_sha256: row.get(0)?, output_uti: row.get(1)?, encoder_version: row.get(2)?, encoder_options_hash: row.get(3)?, width: row_usize(row, 4)?, height: row_usize(row, 5)?, bytes: row.get(6)?,
            }),
        ).optional().context("load targeted representation preview")
    }
